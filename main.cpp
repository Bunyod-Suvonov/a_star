...
// front declare label for class state
class label;
...
class label{
    public:
        ...
        // store the path from origin to current label
...
bool isPrunByFront(label &l, vector<state> &frontier_set){
    ...
    for(int i=0; i<frontier_set[ctr].frontier.size(); i++){
        ...
        // checking for dominance with the characteristics of stored elements
...
void FilterAndAddFront(label &l, vector<state> &frontier_set){
    ...
    for(i=0; i<frontier_set.size(); i++){
        // add label to state's frontier set
...
vector<int> cost(state origin, state successor){
    bool pushed = false;
    // if current pos was the pos of box in origin state, then pushed is true
    ...
    vector<int> cost;
    // movement is definite, so always +1
    cost.push_back(1);
...
// GRAPHICS: initialize the squares in grid
...
// very simple processing given the arguments order is fixed
...
int main(int argc, char *argv[])
{
    ...
    // store current movObj for later display purpose. Used in GRAPHICS section
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::MouseButtonPressed)
            {
                if(event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    // if the button is pressed, path will be drawn and locations of boxes will be updated
